...
class TestReturnTypes:
    ...
    def test_shapes_consistent(self, fif_sims):
        q_values, K_emp, H_fit, C1_fit, alpha_fit = scaleinvariance.K_empirical(
            fif_sims, axis=1)
        assert q_values.shape == K_emp.shape
        assert q_values.ndim == 1

    def test_scalar_outputs(self, fif_sims):
        _, _, H_fit, C1_fit, alpha_fit = scaleinvariance.K_empirical(
            fif_sims, axis=1)
    ...
    def test_default_q_values(self, fif_sims):
        q_values, *_ = scaleinvariance.K_empirical(fif_sims, axis=1)
    ...
    def test_custom_q_values(self, fif_sims):
        ...
        q_values, K_emp, *_ = scaleinvariance.K_empirical(
...
class TestMathematicalIdentities:
    ...
    def test_K_positive_for_q_gt_1(self, fif_sims):
        q_values, K_emp, *_ = scaleinvariance.K_empirical(fif_sims, axis=1)
...
class TestOptions:
    ...
    def test_scaling_method_structure_function(self, fif_sims):
        q, K, H_fit, C1_fit, alpha_fit = scaleinvariance.K_empirical(
...
class TestErrorHandling:
    ...
    def test_invalid_scaling_method_raises(self, fif_sims):
        with pytest.raises(ValueError):
            scaleinvariance.K_empirical(fif_sims, scaling_method='invalid', axis=1)

    def test_invalid_hurst_fit_method_raises(self, fif_sims):
        with pytest.raises(ValueError):
            scaleinvariance.K_empirical(fif_sims, hurst_fit_method='invalid', axis=1)
